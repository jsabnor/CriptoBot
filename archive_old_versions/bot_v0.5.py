...
ATR_LENGTH = 14
ATR_MULTIPLIER = 4.0  # Aumentado para DOGE
ALLOW_REENTRY = True  # Permitir reentrada
USE_MA_FILTER = True  # Filtro MA para entradas
MA_LENGTH = 50
USE_LONG_MA_EXIT = True  # Nuevo: No salir bearish si close > MA(200)
LONG_MA_LENGTH = 200
USE_RSI_FILTER = True  # RSI >60 para entrada
RSI_LENGTH = 14
RSI_THRESHOLD = 60  # Más alto para momentum fuerte
USE_VOLUME_FILTER = False  # Opcional
VOLUME_MA_LENGTH = 20
USE_TRAILING_TP = True  # Trailing take profit
TRAILING_TP_PERCENT = 0.30  # 30% para pumps de DOGE
RISK_PERCENT = 1.0  # All-in
...
def live_strategy():
    ...
    while True:
        limit = max(ATR_LENGTH, MA_LENGTH if USE_MA_FILTER else 0, LONG_MA_LENGTH if USE_LONG_MA_EXIT else 0, RSI_LENGTH if USE_RSI_FILTER else 0, VOLUME_MA_LENGTH if USE_VOLUME_FILTER else 0) + 2
...
if __name__ == "__main__":
    if MODE == 'simulate':
        ...
        limit = days + max(ATR_LENGTH, MA_LENGTH if USE_MA_FILTER else 0, LONG_MA_LENGTH if USE_LONG_MA_EXIT else 0, RSI_LENGTH if USE_RSI_FILTER else 0, VOLUME_MA_LENGTH if USE_VOLUME_FILTER else 0) + 10
