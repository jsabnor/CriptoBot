...
TIMEFRAME = '1d'  # Temporalidad: '1m', '5m', '1h', '1d', '1w', etc.
...
ATR_LENGTH = 14
ATR_MULTIPLIER = 3.0  # Aumentado para menos hits (prueba 4.0)
ALLOW_REENTRY = True  # Permitir reentrada tras cierre (poco impacto en all-in)
USE_MA_FILTER = True  # Nuevo: Filtro MA para entradas (close > SMA(50))
MA_LENGTH = 50
...
def live_strategy():
    ...
    while True:
        df = fetch_ohlcv(SYMBOL, TIMEFRAME, limit=max(ATR_LENGTH, MA_LENGTH if USE_MA_FILTER else 0) + 2)
        df = calculate_atr(df)
        if USE_MA_FILTER:
            df = calculate_ma(df)
...
if __name__ == "__main__":
    if MODE == 'simulate':
        ...
        limit = days + max(ATR_LENGTH, MA_LENGTH if USE_MA_FILTER else 0) + 10
