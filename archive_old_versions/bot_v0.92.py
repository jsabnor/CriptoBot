...
SYMBOL = 'DOGE/USDT'  # DOGE/USDT
TIMEFRAME = '4h'  # Temporalidad
INITIAL_CAPITAL = 200.0  # Capital inicial en USDT (cambia si necesitas)
COMMISSION = 0.001  # Comisión por trade (0.1% = 0.001)
USE_ATR_SL = True  # Usar SL basado en ATR
ATR_LENGTH = 14
ATR_MULTIPLIER = 5.0  # Amplio
ALLOW_REENTRY = True  # Desactivado para no permitir reentradas
...
RISK_PERCENT = 1.0  # All-in (ajusta si quieres)
...
def simulate_strategy(df, initial_capital):
    ...
    for i in range(1, len(df)):
        ...
        if can_enter:
            ...
            min_qty = 100  # Min para DOGE/USDT en Binance (ajusta)
            min_notional = 10.0  # Min valor en USDT
            if qty < min_qty or qty * close < min_notional:
                continue  # Salta buy tiny/residual
            if qty > 0:
                position_size += qty
                equity -= qty * close
