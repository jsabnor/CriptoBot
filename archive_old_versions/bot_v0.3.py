...
ATR_LENGTH = 14
ATR_MULTIPLIER = 3.0  # Para menos hits
ALLOW_REENTRY = True  # Permitir reentrada
USE_MA_FILTER = True  # Filtro MA para entradas
MA_LENGTH = 50
USE_RSI_FILTER = True  # Nuevo: Filtro RSI > 50
RSI_LENGTH = 14
RSI_THRESHOLD = 50
USE_VOLUME_FILTER = False  # Opcional: Volume > MA(volume)
VOLUME_MA_LENGTH = 20
USE_TRAILING_TP = True  # Nuevo: Trailing take profit
TRAILING_TP_PERCENT = 0.10  # 10% retroceso desde max
RISK_PERCENT = 0.50  # Nuevo: % de equity por trade (no all-in)
...
def calculate_rsi(df, length=RSI_LENGTH):
    ...
    rs = gain / loss
    df['rsi'] = 100 - (100 / (1 + rs))
    df['rsi'] = df['rsi'].fillna(50)  # Neutral inicial
    return df
...
def simulate_strategy(df, initial_capital):
    ...
    position_size = 0.0
    sl_price = 0.0
    max_price = 0.0  # Para trailing TP
    trades = []
    in_position = False
    ...
    for i in range(1, len(df)):
        ...
        # Actualiza max_price y chequea trailing TP
        if in_position:
            max_price = max(max_price, high)
            tp_price = max_price * (1 - TRAILING_TP_PERCENT)
            if USE_TRAILING_TP and close <= tp_price:
                sell_price = close
        ...
        if can_enter:
            ...
            qty = risk_equity / close  # Simplificado; para risk-based full: / (close - (close - atr * ATR_MULTIPLIER))
            if qty > 0:
                position_size += qty
                equity -= qty * close * (1 + COMMISSION)  # Incluye comisión en buy
                sl_price = close - atr * ATR_MULTIPLIER if USE_ATR_SL else 0
                max_price = close  # Reset para TP
