...
class NeuralTradingModel:
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, epochs=None):
        ...
        # Usar class weights configurados (prioriza BUY para más señales)
        class_weight_dict = config.CLASS_WEIGHTS.copy()
        
        print(f"\n⚖️ Class Weights (configurados para priorizar BUY):")
...
class ContinuousLearner:
    ...
    def prepare_training_data(self, symbols=None, timeframe='4h', start_date=None, end_date=None):
        ...
        2. Filtra por fechas si se especifican
        3. Encuentra el mínimo de muestras para balancear
        4. Hace split train/val POR SÍMBOLO
        5. Concatena y mezcla (shuffle)
        
        Args:
            symbols: Lista de símbolos
            timeframe: Timeframe
            start_date: Fecha inicio (YYYY-MM-DD) o None
            end_date: Fecha fin (YYYY-MM-DD) o None
        ...
        for i, symbol in enumerate(symbols):
            ...
            if start_date or end_date:
                ...
                if len(df) < config.MIN_TRAIN_SAMPLES:
                    print(f"    ⚠️ Datos insuficientes después de filtrar por fechas")
                    continue
    ...
    def train_initial_model(self, symbols=None, timeframe='4h', start_date=None, end_date=None):
